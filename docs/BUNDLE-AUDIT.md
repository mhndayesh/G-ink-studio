...
> - **#7 (non-canonical character spelling)** — partially mitigated: the visuals bundle now lowercases dialogue speaker labels so they match the (also-lowercased) character reference sheets and the importer's lowercase id lookup. **Still fix at source** — the export can't repair typo-level drift (`oddo` vs `Odo`); use one canonical spelling per character across id / display name / dialogue label / prose, with explicit aliases for short forms.
...
>
> Still **upstream-only**: improving the LLM *generation* so the stored prompts/locations are already clean — the prompt schemas in `llm_service.py` now ask the model for short, visual-only, colour-free, style-word-free output (`STYLE_INSTRUCTION`), but old stored data is only cleaned on export.
