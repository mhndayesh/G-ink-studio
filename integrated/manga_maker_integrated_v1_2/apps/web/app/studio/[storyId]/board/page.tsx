...
const ARC_LENGTH_OPTS = ["One-Shot", "Short Arc", "Medium Arc", "Long Arc", "Saga", "Season", "Full Series", "Custom"];
const ARC_LENGTH_SPECS: Record<string, { min: number; ideal: number; max: number; label: string }> = {
...
const STRUCTURE_BEATS: Record<string, Array<{ key: string; label: string; purpose: string }>> = {
  "Mystery Arc": [
...
function isMeaningfulChapter(chapter: any): boolean {
  return CHAPTER_CONTENT_FIELDS.some((field) => hasContent(chapter?.[field]));
}

function selectedOptionValue(value: any): string {
  let current = value;
  for (let i = 0; i < 4; i += 1) {
